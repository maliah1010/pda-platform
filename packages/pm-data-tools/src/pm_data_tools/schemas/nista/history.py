"""NISTA compliance score history, trend analysis, and threshold alerting.
...
Usage::
    ...
    result = validator.validate(data, project_id="PROJ-001", history=history)

    trend = history.compute_trend("PROJ-001")
    breaches = history.check_thresholds("PROJ-001")
"""
...
class NISTAScoreHistory:
    ...
    Example::
        ...
        history.record(ConfidenceScoreRecord(
            project_id="PROJ-001",
            score=78.5,
        ))
        trend = history.compute_trend("PROJ-001")
    ...
    def __init__(
        ...
        thresholds: Optional[NISTAThresholdConfig] = None,
    ) -> None:
        """Initialise the score history manager.

        Args:
            store: Shared SQLite store.  A default store is created if not
                provided.
            thresholds: Threshold configuration for breach detection.
                Defaults to :class:`NISTAThresholdConfig` defaults.
        """
        self._store = store or AssuranceStore()
        self._thresholds = thresholds or NISTAThresholdConfig()
